...
| 状态 | 场景 | 预期结果 | 备注 |
| --- | --- | --- | --- |
| ⬜ | 用户取消当前订阅 | Stripe 设置为周期末取消；权益保留到本周期结束 | 下一步建议优先测这一项 |
| ⬜ | 取消后查看 Profile | Profile 显示取消状态和权益有效期；支付方式仍可见 | 待测 |
| ⬜ | 取消后继续访问权益页 | 周期结束前仍可使用已购买权益 | 待测，可配合 Stripe Test Clock |
...
| 状态 | 场景 | 预期结果 | 备注 |
| --- | --- | --- | --- |
| ⬜ | 未登录用户访问 `/deliver-to/nl` 或 `/deliver-to/fr` | 不展示库存数据；引导登录/订阅 | 待测 |
| ⬜ | 无订阅用户访问 `/deliver-to/nl` 或 `/deliver-to/fr` | 不展示库存数据；引导订阅 | 待测 |
| ⬜ | `basic` 用户访问实时库存页 | 不展示库存数据；提示当前方案仅包含邮件提醒 | 待测 |
| ⬜ | `priority` 用户访问实时库存页 | 根据用户国家字段进入 `/deliver-to/nl` 或 `/deliver-to/fr` 并展示可配送站点 | 待测 |
